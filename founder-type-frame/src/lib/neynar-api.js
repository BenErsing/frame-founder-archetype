...
/**
 * Server-side function to fetch user profile from Neynar API
 * This should only be called from the server-side, never from client code.
 * 
 * @param {number} fid - The FID of the user to fetch profile for
 * @returns {Promise<Object>} - Object containing user profile information
 */
export async function getUserProfile(fid) {
  ...
  const response = await fetch(`${NEYNAR_API_BASE_URL}/user/bulk?fids=${fid}`, {
    headers: {
